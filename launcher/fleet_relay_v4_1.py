...
def process_message_with_retry(r, agent: str, payload_str: str, *, max_retries: int = 3, is_recovery: bool = False) -> str:
    """
    Traite un message avec politique de tentatives limitées.
    Après max_retries échecs, le message va en DLQ.
    """
    msg_id = "?"
    try:
    ...
    # Vérifier si déjà en DLQ
    if not should_process_message(r, agent, msg_id):
        log(f"SKIP msg_id={msg_id} (in DLQ)", agent=agent)
        return "IN_DLQ"

    # Compter les tentatives actuelles
    attempt_key = f"attempt:{agent}:{msg_id}"
    attempts = int(r.get(attempt_key) or 0)

    if attempts >= max_retries:
        # Épuisement des tentatives → DLQ
        error = f"Max retries ({max_retries}) exceeded"
        log(f"DLQ msg_id={msg_id} after {attempts} attempts", agent=agent, level="ERROR")
        send_to_dlq(r, agent, payload_str, error, attempts)
        r.delete(attempt_key)
        return "DLQ"

    # Incrémenter le compteur de tentatives
    r.set(attempt_key, attempts + 1, ex=3600)

    # Traiter le message
    status = process_message(r, agent, payload_str, is_recovery=is_recovery)

    # En cas d'erreur, remettre dans la queue pour réessayer plus tard
    if status in ("TIMEOUT", "ERROR"):
        log(f"RETRY msg_id={msg_id} attempt {attempts + 1}/{max_retries}", agent=agent)
        # Remettre dans la queue pour réessayer
        r.rpush(f"queue:{agent}", payload_str)
        return status

    # Succès → nettoyer le compteur
    r.delete(attempt_key)
    return status


def process_message(r, agent: str, payload_str: str, *, is_recovery: bool = False) -> str:
    """Traite un message (une seule tentative)."""
    try:
        msg = json.loads(payload_str)
    ...
    try:
        ...
        # Si le launcher échoue (exit != 0), ne PAS écrire msg:done
        # pour permettre la nouvelle tentative
        if result.returncode != 0:
            error = result.stderr.strip() or f"exit={result.returncode}"
            log(f"LAUNCH_ERROR task_id={task_id} error={error}", agent=agent, level="ERROR")
            return "ERROR"
...
def should_process_message(r, agent: str, msg_id: str) -> bool:
    """
    Vérifie si un message doit être traité.
    Retourne False si le message est en DLQ.
    """
    dlq_key = f"dlq:{agent}"
    dlq_entries = r.lrange(dlq_key, 0, -1)
    for entry_str in dlq_entries:
        try:
...
def send_to_dlq(r, agent: str, payload_str: str, error: str, attempts: int = 1):
    """
    Envoie un message dans la Dead Letter Queue avec ses métadonnées.
    """
    dlq_key = f"dlq:{agent}"
    try:
        msg = json.loads(payload_str)
    except json.JSONDecodeError:
        msg = {"raw": payload_str[:500]}
    ...
    dlq_entry = {
        ...
        "error": error,
        "attempts": attempts,
        "original_msg": payload_str,
        "ts": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "agent": agent,
    }
    r.rpush(dlq_key, json.dumps(dlq_entry))
    r.expire(dlq_key, 604800)  # 7 jours
    log(f"DLQ_PUSH msg_id={dlq_entry['msg_id']} error={error}", agent=agent)


def recover_from_dlq(r, agent: str, msg_id: str) -> bool:
    """
    Récupère un message depuis la DLQ et le remet dans la queue.
    Retourne True si le message a été récupéré.
    """
    dlq_key = f"dlq:{agent}"
    queue_key = f"queue:{agent}"

    # Chercher le message dans la DLQ
    dlq_entries = r.lrange(dlq_key, 0, -1)
    for i, entry_str in enumerate(dlq_entries):
        try:
            entry = json.loads(entry_str)
            if entry.get("msg_id") == msg_id:
                # Remettre dans la queue
                original_msg = entry.get("original_msg", "")
                if original_msg:
                    r.rpush(queue_key, original_msg)
                    # Supprimer de la DLQ (remplacer par marqueur)
                    r.lset(dlq_key, i, '{"recovered": true, "msg_id": "' + msg_id + '"}')
                    log(f"DLQ_RECOVER msg_id={msg_id} restored to queue", agent=agent)
                    return True
        except json.JSONDecodeError:
            continue
    return False
    ...
    while running:
        for agent in agents:
            ...
            try:
                ...
                # Traitement
                status = process_message_with_retry(r, agent, payload_str, max_retries=3)
